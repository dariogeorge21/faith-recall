...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  addGame1Score: (points) => set((state) => ({ 
    game1Score: Math.max(0, state.game1Score + points),
    game1Matches: state.game1Matches + 1
  })),
  
  addGame1Penalty: (points) => set((state) => ({ 
    game1Score: state.game1Score - points 
  })),
  
  incrementGame1Combo: () => set((state) => ({ 
    game1Combo: Math.min(5, state.game1Combo + 1) 
  })),
  
  resetGame1Combo: () => set({ game1Combo: 0 }),
  
  addGame2Score: (points) => set((state) => ({ 
